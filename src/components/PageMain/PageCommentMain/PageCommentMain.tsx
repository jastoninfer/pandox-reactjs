...
import type { PaginatedThreadsResData, SinglePaginatedThread } from 'types/thread';
import type { AuthState, _ReduxState } from 'types/states';
import type { PaginatedCommentsResData, SinglePaginatedComment } from 'types/comment';
import type { PaginatedESPagesResData, PaginatedESUsersResData } from 'types/search.es';
...
const PageSubCommentItem:React.FC<PageSubCommentItemProps> = ({ comment, handleReplyButtonOnClick, handleDeleteCommentOnClick}) => {
        ...
        <S.BlogSubCommentItem>
            ...
            <div className='comment-tools'>
                ...
                <div className='comment-tools-ops'>
                    {isLoggedin && user && (user.username===comment.from) && (<a className='delete-comment' onClick={handleDeleteCommentOnClick(comment.id)}>Delete</a>)}
                    {/* 如果是已登录状态，应该允许回复 */}
...
interface PageCommentItemProps {
    ...
    activeReplyBox: {
        ...
        setActiveReplyBoxThreadIdx: React.Dispatch<React.SetStateAction<number>>,
    };
    handleDeleteThreadOnClick: (threadId: number) => () => Promise<void>;
}

const PageCommentItem:React.FC<PageCommentItemProps> = ({ thread, activeReplyBox, handleDeleteThreadOnClick}) => {
    // 这里定义一个Thread
    // thread maybe undefined
    const threadId:number = thread.id;
    const [comments, setComments] = useState<PaginatedCommentsResData|null>(null);
    ...
    const handleReplyBoxOnChange = (e:React.ChangeEvent<HTMLTextAreaElement>) => {
        ...
        <S.BlogCommentItem>
            ...
            <div className='Blog-Comment-Item-Body'>
                ...
                <div className='comment-tools'>
                    ...
                    <div className='comment-tools-ops'>
                        {comments && comments.comments.length > 0 && (hideComments ? 
                        ...
                        {isLoggedin && user && (user.username===thread.author) && (<a className='delete-comment' onClick={handleDeleteThreadOnClick(threadId)}>Delete</a>)}
                        {/* 如果是已登录状态，应该允许回复 */}
                        {isLoggedin && (<a onClick={handleReplyButtonOnClick(thread.author)}>Reply</a>)}
                    </div>
                </div>

                { !hideComments && comments && comments.comments.map(comment => (
                <PageSubCommentItem
            ...
            {!hideComments && comments && comments.total>1&& (<PaginationNavBar displaytype='subcomment' items={comments} getItemsByIdx={getComments}/>)}
        </S.BlogCommentItem>
    );
};
...
interface PaginationNavBarProps {
    ...
    items: PaginatedThreadsResData|PaginatedCommentsResData|ProfileBlogsData|PaginatedESUsersResData|PaginatedESPagesResData;
}

export const PaginationNavBar:React.FC<PaginationNavBarProps> = ({ displaytype, getItemsByIdx, items}) => {
    ...
    const handlePaginationIndexClick = (e:React.MouseEvent<HTMLSpanElement>) => {
...
const PageCommentMain = React.forwardRef<PageMainCommentForwardRef, PageCommentMainProps>((props, ref) => {

    const pageId:number|undefined = props.pageId;// 永远只记加载/处理第一篇博客下的threads

    const [threads, setThreads] = useState<PaginatedThreadsResData|null>(null);
    const [activeReplyBoxThreadIdx, setActiveReplyBoxThreadIdx] = useState<number>(-1);
    const [isMainReplyBoxOpen, setIsMainReplyBoxOpen] = useState<boolean>(false);
    ...
    const handleMakeBlogCommentOnClick = (e: React.MouseEvent<HTMLButtonElement>):void => {
        window.scrollTo({
    ...
    const handleMainReplyBoxValueOnChange = (e:React.ChangeEvent<HTMLTextAreaElement>) => {
        setMainReplyBoxValue(e.target.value);
    };

    const handleMainReplyCancelOnClick = (e:React.MouseEvent<HTMLButtonElement>) => {
        // 关闭输入框
    ...
    const getThreads = async (threadPageId:number=1) => {
        try {
            if(props.pageId) {
                const data:PaginatedThreadsResData = await ThreadService.getThreadsByPageId(props.pageId, threadPageId);
                setThreads(data);
            }
        } catch (err:any){
    ...
    const getThreadsAndScroll = async (threadPageId:number=1) => {
        ...
        const blogCommentBody:HTMLElement|null = document.querySelector('#BlogCommentBody');
        if(blogCommentBody) {
            window.scrollTo({top: blogCommentBody.getBoundingClientRect().top + window.scrollY-100});
        }
    }
    ...
    return (
            <S.BlogCommentBody id='BlogCommentBody'>
                ...
                {threads && threads.total > 1 && (<PaginationNavBar items={threads} getItemsByIdx={getThreadsAndScroll}/>)}
                {isMainReplyBoxOpen && (
