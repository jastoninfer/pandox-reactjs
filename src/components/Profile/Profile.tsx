...
import { NavLink, NavigateFunction, Outlet, useParams,useNavigate, useLocation } from 'react-router-dom';
...
import type { SingleProfileBlogData, ProfileBlogsData,PaginatedPagesResData } from 'types/page';
...
const ProfileContext = createContext<ProfileContextType>({user:null, authUser:null});
...
export const ProfileBlogs = () => {
    ...
    const getPages = async (idx=1):Promise<ProfileBlogsData|undefined> => {
        ...
        try {
            const pageData:PaginatedPagesResData = await PageService.getPagesByOwner(user.username, idx);
    ...
    const getPagesAndScroll = async (idx:number=1) => {
        ...
        if(pagesData) {
            setPages(pagesData);
            const profileBlogsContainer:HTMLElement|null = document.querySelector('.profile-blogs-container');
            if(profileBlogsContainer) {
                window.scrollTo({behavior: 'smooth', top: profileBlogsContainer.getBoundingClientRect().top + window.scrollY-100});
            }
        }
    }
    ...
    const handleBlogAuthorOnClick = (pageItem:SingleProfileBlogData) => (e:React.MouseEvent<HTMLDivElement>) => {
        e.preventDefault();
        navigate(`/users/@${pageItem?.author}`);
    };

    const handleBlogContentOnClick = (pageItem:SingleProfileBlogData) => (e:React.MouseEvent<HTMLDivElement>) => {
        e.preventDefault();
...
const Profile = () => {
    const avatar = useSelector<_ReduxState, string|undefined>(state => state.auth.user?.avatar);
    const authUser = useSelector<_ReduxState, User_w_Token|null>(state => state.auth.user);
    // URL: /users/:username
    ...
    const getUserProfile = async () => {
        ...
        try {
            const userData:User = await UserSerivce.viewProfile(username.slice(1));
            setUser(userData);
        } catch (err:any) {
            console.log(err.message || 'Error occurred.');
            return Promise.reject('User not found.');
    ...
    const handleEditProfileOnClick = (e:React.MouseEvent<HTMLAnchorElement>) => {
        e.preventDefault();
