...
): Promise<ThreadResData> => {
    return http
        .post(`${BASE_URL}threads/${pageId}`, threadData, {
            headers: authHeader(),
        })
...
const getThreadsByPageId = (
    pageId: number,
    threadIdx: number = 1
): Promise<PaginatedThreadsResData> => {
    return http
        .get(`${BASE_URL}threads/${pageId}?threadIdx=${threadIdx}`)
...
const deleteThreadById = (threadId: number): Promise<_MessageResData> => {
    return http
        .delete(`${BASE_URL}threads/${threadId}`, { headers: authHeader() })
