...
const getKeywordsByPageId = (pageId: number): Promise<PageKeywordResData[]> => {
    // 可选校验身份
    return http
        .get(`${BASE_URL}/${pageId}`, { headers: authHeader() })
        .then((res: PageKeywordsResponse) => {
            return res.data;
        });
};
...
const createKeyword = (
    pageId: number,
    keyword: string
): Promise<PageKeywordResData> => {
    // keyword应该是纯文本, 包装成json后发送出去
    return http
        .post(`${BASE_URL}/${pageId}`, { keyword }, { headers: authHeader() })
        .then((res: PageKeywordResponse) => {
            return res.data;
        });
};

const deleteKeywordByPageId = (
    pageId: number,
    keyword: string
): Promise<_MessageResData> => {
    return http
        .delete(`${BASE_URL}/${pageId}/${keyword}`, { headers: authHeader() })
