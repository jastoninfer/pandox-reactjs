...
const getPageById = (pageId: number): Promise<PageResData> => {
    // 可选校验身份
    return http
        .get(`${BASE_URL}pages/${pageId}`, { headers: authHeader() })
...
const createPage = (pageData: CreatePage): Promise<PageResData> => {
    // 必须校验身份
    return http
        .post(`${BASE_URL}pages`, pageData, { headers: authHeader() })
        .then((res: PageResponse) => {
            return res.data;
        });
};
...
const getRecommendedPages = (): Promise<PageRecommendResData[]> => {
    ...
    // 同时获取对应的page的用户的头像
    return http
        .get(`${BASE_URL}pagerecommend`, { headers: authHeader() })
        .then((res: PagesRecommendResponse) => {
            return res.data;
        });
};

const getPagesByOwner = (
    pageOwner: string,
    pageIdx: number = 1
): Promise<PaginatedPagesResData> => {
    return http
        .get(`${BASE_URL}pagelist?pageOwner=${pageOwner}&pageIdx=${pageIdx}`, {
            headers: authHeader(),
        })
        .then((res: PaginatedPagesResponse) => {
            return res.data;
        });
};
...
const updatePageById = (
    pageId: number,
    newPageData: UpdatePage
): Promise<_MessageResData> => {
    // 必须校验身份
    return http
        .put(`${BASE_URL}pages/${pageId}`, newPageData, {
            headers: authHeader(),
        })
...
const deletePageById = (pageId: number): Promise<_MessageResData> => {
    // 必须校验身份
    return http
        .delete(`${BASE_URL}pages/${pageId}`, { headers: authHeader() })
        .then((res: AxiosResponse<_MessageResData>) => {
            return res.data;
        });
};
